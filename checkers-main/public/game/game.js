...
/*----------- Options ----------*/
const { player1, player2, isWhite, room } = Qs.parse(window.location.search, {
...
class Player {
  constructor({ id, username, rating }, color) {
    ...
    this.rating = rating;
    this.color = color;
    this.arsenal = document.querySelectorAll(
      color ? ".white-piece" : ".black-piece"
    );
  }
}
...
const givePieceAndMovesColor = (event) => {
  ...
  for (let i = 0; i < allAvailbleMoves.length; i++)
    if (indexClickedPiece === allAvailbleMoves[i].startLocation)
      cells[allAvailbleMoves[i].endLocation].style =
...
const makeMove = (event) => {
  ...
  if (
    playerTurn
      ? move.endLocation >= 0 && move.endLocation <= 7
      : move.endLocation >= 56 && move.endLocation <= 63
  )
...
const updateCells = (piece, move) => {
  if (board[move.startLocation].isKing)
    cells[move.endLocation].innerHTML = `<p class="${
      playerTurn ? "white-piece king" : "black-piece king"
    }" id="${piece.id}"></p>`;
  else
    cells[move.endLocation].innerHTML = `<p class="${
      playerTurn ? "white-piece" : "black-piece"
    }" id="${piece.id}"></p>`;
  ...
  for (let i = 0; i < allAvailbleMoves.length; i++)
    if (allAvailbleMoves[i].isMandatory) countMandatoryMoves++;
...
const canMove = (startLocation, endLocation) => {
  if (!board[startLocation].isKing) {
    ...
    if (
      ...
      return true;
    if (
      endLocation - startLocation === direction9 &&
      board[endLocation] === null
    )
      return true;
...
const checkArsenalLength = () => {
  if (
    (playerTurn ? whitePlayer.arsenal.length : blackPlayer.arsenal.length) === 0
  ) {
    isGameOver = true;
    return true;
  }
...
const gameOver = (resignedPlayer) => {
  ...
  if (resignedPlayer) {
    winner =
      resignedPlayer === whitePlayer.username
    ...
    winner = isDraw
      ...
      ? blackPlayer.username
      : whitePlayer.username;
    gameOverModal.firstElementChild.children[0].innerHTML = isDraw
      ? "DRAW!"
      : `${winner} WON!`;
  }
  let data;
  if (winner === playerData.username || winner === "DRAW")
    data = { wins: playerData.rating.wins + 1 };
  else data = { losses: playerData.rating.losses + 1 };

  fetch("/users/me/record", {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      console.log(data);
      const gameOverModalRating = document.getElementById(
...
gameResignButton.addEventListener("click", () => {
  const resignedPlayer = currentPlayer
    ? whitePlayer.username
    : blackPlayer.username;
...
gameDrawButton.addEventListener("click", () => {
  socket.emit("drawRequest", room, () =>
    console.log("Your draw request was sent!")
  );
...
gameOverHomeButton.addEventListener("click", () => {
  socket.emit("lobby", () =>
    console.log("Both players are redirecting back to the lobby!")
  );
});
gameOverRematchButton.addEventListener("click", () => {
  socket.emit("rematchRequest", () =>
    console.log("Your rematch request was sent!")
  );
...
socket.on("recieveDrawRequest", () => {
  drawModal.firstElementChild.children[0].innerHTML =
    playerTurn === true
      ? `${blackPlayer.username}, DO YOU ACCEPT?`
      : `${whitePlayer.username}, DO YOU ACCEPT?`;
...
socket.on("newGame", () => {
  ...
  while (boardElement.firstElementChild)
